...
def fetch_pitcher_statcast(year: int = YEAR) -> pd.DataFrame:
    """
    Pull pitcher-level Statcast metrics from Baseball Savant leaderboard.
    Returns wOBA, xwOBA, hard_hit%, whiff%, k%, bb% etc per pitcher.
    """
    url = (
        "https://baseballsavant.mlb.com/leaderboard/custom"
        f"?year={year}&type=pitcher&filter=&sort=2&sortDir=asc"
        "&min=10"
