...
def main():
    ...
    target = "2026-07-18"  # TEMP: hardcoded for validation run
    ...
    # TEMP VALIDATION: dump the fetched+keyed results dict directly, so the
    # fix can be verified against real historical data without depending on
    # game_log.csv's current row completeness. Remove after validation.
    with open("data/validation_dump.txt", "w") as vf:
        for key, r in sorted(results.items()):
            vf.write(f"key={key}  away_score={r['away_score']} home_score={r['home_score']} "
                     f"away_f5={r['away_f5']} home_f5={r['home_f5']}\n")
